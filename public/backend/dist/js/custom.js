$(document).ready(function() {
	...
	var $fileuploader = $('input.gallery_media').fileuploader({
		limit: 100,
		fileMaxSize: 50,
		extensions: ['png','jpg','jpeg','avi','mp4'],
		changeInput: ' ',
		theme: 'gallery',
		enableApi: true,
		thumbnails: {
		...
		upload: {
			url: './dist/uploader/ajax.php?type=upload',
			data: null,
			type: 'POST',
		...
		editor: {
			...
			onSave: function(dataURL, item) {
				...
				if (item.appended && item.data.listProps) {
					...
					$.post('dist/uploader/ajax.php?type=resize', {name: item.name, id: item.data.listProps.id, _editor: JSON.stringify(item.editor)}, function() {
						// update the image
		...
		sorter: {
			onSort: function(list, listEl, parentEl, newInputEl, inputEl) {
				...
				// send request
				$.post('dist/uploader/ajax.php?type=sort', {
			...
			$('body').on('click', function(e) {
				...
				if ($target.is('.gallery-action-rename')) {
					...
					if (x && item.data.listProps) {
						$.post('dist/uploader/ajax.php?type=rename', {name: item.name, id: item.data.listProps.id, title: x}, function(result) {
							try {
				...
				if ($target.is('.gallery-action-asmain') && item.data.listProps) {
					$.post('dist/uploader/ajax.php?type=asmain', {name: item.name, id: item.data.listProps.id}, function() {
		...
		onRemove: function(item) {
			// send request
			if (item.data.listProps)
				$.post('dist/uploader/ajax.php?type=remove', {
					name: item.name,
					id: item.data.listProps.id
				});
		},
		captions: $.extend(true, {}, $.fn.fileuploader.languages['en'], {
			feedback: 'Foto seçin & Buraya sürüşdürün',
			setting_asMain: 'Use as main',
			setting_download: 'Download',
			setting_edit: 'Düzəliş',
			setting_open: 'Aç',
			setting_rename: 'Adı dəyiş',
	...
	// preload the files
	$.post('dist/uploader/ajax.php?type=preload', null, function(result) {
